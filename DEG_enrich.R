...
library(org.PParaescarpia.eg.db)
library(ggplot2)
library(enrichplot)
library(stringr)
args <- commandArgs(T)
...
ego <- enrichGO(gene          = genes, # list of entrez gene id
                OrgDb         = org.PParaescarpia.eg.db, # 背景使用分析物种的org包
                keyType       = 'GID', 
...
go_bar <- ggplot(data = go.df, aes(x = Description, y = -log10(pvalue),fill = ONTOLOGY))+geom_bar(stat = "identity",width = 0.9)+coord_flip()+theme_bw()+scale_x_discrete(labels = function(x) str_wrap(x,width = 50))+labs(x = "GO terms",y = "-log10(pvalue)",title = "Barplot of Enriched GO Terms")+theme(axis.title = element_text(size = 13),axis.text = element_text(size = 11),plot.title = element_text(size = 14,hjust = 0.5,face = "bold"),legend.title = element_text(size = 13),legend.text = element_text(size = 11),plot.margin = unit(c(0.5,0.5,0.5,0.5),"cm"))
...
kegg_anno <- read.table("/dellfsqd2/ST_OCEAN/USER/zhangxianghui/05.Deep_sea/1.Riftia_achyptila/quality_control/eggnog/gene2pathway_name.txt",sep="\t",header=T)
